...
func (pb *parserBuffer) NextChunk(n int) string {
...
func (pb *parserBuffer) manage() {
	...
	// Handle any pending requests than we have enough buffer data to deal with,
	// and then close the response chans for any remaining requests.
	close(pb.requestsIn)
	handleRequests()
